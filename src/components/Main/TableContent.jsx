...
const TableContent = (props) => {
 ...
 const navigate = useNavigate();
  let priceChangePercentage = props.coinDetail.data.price_change_percentage_24h?.usd;
  priceChangePercentage = priceChangePercentage?.toFixed(2);
  if (!priceChangePercentage) {
    priceChangePercentage = 0;
  }

  let symbol = props.coinDetail.symbol.toUpperCase();
  
  let price = props.coinDetail.data?.price.toLocaleString();
  if (!price) {
    price = 0;
  }
   
  let totalVol = props.coinDetail.data.total_volume?.toLocaleString();
  if (!totalVol) {
    totalVol = 0;
  }

  let marktCap = props.coinDetail.data.market_cap?.toLocaleString();
  if (!marktCap) {
    marktCap = 0;
  }
  return (
    <tr onClick={() => navigate(`/coins/${props.coinDetail?.id}`)} className={`border-b-[1px] ${
